...
def getArgs():
    ...
    parser.add_argument(
        "--fetch_interval", help="Interval to read piaware status", default=15, type=int
    )
    parser.add_argument(
        "--secure", help="Use HTTPS to access PiAware Status", action="store_true"
    )
...
def main():
    ...
    if args.secure or args.piaware_port == "443":
        proto = "https"
    else:
        proto = "http"
    ...
    piaware_exporter = PiAwareMetricsExporter(
        args.piaware_host, args.piaware_port, args.fetch_interval, proto
    )
    ...
    try:
        ...
        logger.error(
            f"Could not start Prometheus server endpoint on port {args.expo_port}: {e}"
        )
        sys.exit(1)
